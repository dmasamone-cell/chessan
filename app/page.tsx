...
export default function Page() {
  ...
  // Konversi Centipawns ke Win Percentage (Sigmoid Chess.com / Lichess)
  ...
  async function analyzeGame(g: GameItem) {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (side === 'w') {
        totalWinLossW += winPctLoss;
        countW++;
      } else {
        totalWinLossB += winPctLoss;
        countB++;
      }
    ...
    // Formula Presisi Akurasi Chess.com (Harus dapat nilai rendah kalau banyak mistake/blunder)
    const avgLossW = countW > 0 ? totalWinLossW / countW : 0;
    const avgLossB = countB > 0 ? totalWinLossB / countB : 0;

    const finalAccW = Math.max(15, Math.min(99, 100 - avgLossW * 2.2)).toFixed(1);
    const finalAccB = Math.max(15, Math.min(99, 100 - avgLossB * 2.2)).toFixed(1);
